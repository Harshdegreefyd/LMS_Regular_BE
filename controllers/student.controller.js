...
export const updateStudentStatus = async (req, res) => {
  try {
    ...
    if (callingStatus === "Not Connected") {
      console.log(`📞 Not Connected detected for student ${studentId}`);

      try {
        const today = new Date().toISOString().split("T")[0];

        // FIXED: Use Sequelize instead of pool
        const todayRemarks = await StudentRemark.findAll({
          where: {
            student_id: studentId,
            [Op.and]: [
              sequelize.where(
          ...
          attributes: ["calling_status", "created_at"],
        });

        const totalRemarksToday = todayRemarks.length;

        if (totalRemarksToday === 1) {
          console.log(
            `✅ This is the FIRST remark of the day for ${studentId}`,
          );

          const created_at = student.created_at;
          const studentCreatedDate = new Date(created_at);
          const todayDate = new Date();

          const timeDiff = todayDate.getTime() - studentCreatedDate.getTime();
          const daysDifference = Math.floor(timeDiff / (1000 * 60 * 60 * 24));

          console.log(`📅 Days since student creation: ${daysDifference}`);
          ...
          if (daysDifference >= 0 && daysDifference <= 4) {
            ...
            switch (daysDifference) {
              case 0:
                ...
                break;
              case 1:
                templateId = "not_connected_template_day2"; // DAY_1
                break;
              case 2:
                templateId = "not_connected_template_day3"; // DAY_2
                break;
              case 3:
                templateId = "not_connected_template_day4"; // DAY_3
                break;
              case 4:
                templateId = "not_connected_template_day5"; // DAY_4
                break;
            }

            if (templateId && student.student_phone) {
              console.log(`📤 Sending WhatsApp for Day ${daysDifference}`);
              console.log(`📋 Using template: ${templateId}`);

              // FIXED: Update sendNotInterestedFollowup function
              const result = await sendNotInterestedFollowup(
                studentId,
                student.student_phone,
                daysDifference === 0 ? 1 : daysDifference, // Send day number
                templateId, // Use the specific template
              );

              if (result.success) {
                console.log(
                  `✅ WhatsApp sent successfully to ${student.student_name}`,
                );
              } else {
            ...
            console.log(
              `⏭️ Student is ${daysDifference} days old - No WhatsApp beyond Day 5`,
            );
          }
        } else if (totalRemarksToday > 1) {
          const firstRemarkToday = todayRemarks[0];
          const firstCallingStatus = firstRemarkToday.calling_status;

          if (firstCallingStatus !== "Not Connected") {
            console.log(
              `⏭️ Not sending - First remark was "${firstCallingStatus}"`,
            );
          } else {
            console.log(
              `⏭️ Not sending - Already sent for first "Not Connected" remark`,
            );
          }
        }
      } catch (whatsappError) {
        console.error(`❌ Error in WhatsApp sending:`, whatsappError.message);
      }
    }
